...
\\
\\
When unrelated logic becomes scattered throughout a program, we see an evolution of that logic into a cross-cutting concern. We say that it is cross-cutting because it cuts across multiple components of a program without belonging to any single one of them. This tangling \cite{aop-ECCOP} of cross-cutting concerns in a system becomes problematic for many reasons, two of which I will highlight below due to their relevance to our project
\begin{enumerate}
...
Looking back \verb InstantiationAspect, we have the statement
\begin{verbatim}
    pointcut new_exec(): execution(new(..));
\end{verbatim}
...
\\
Advice in AspectJ compactly described in the informal EBNF below
\begin{verbatim}
    <Advice> ::= <Before>
               | <After>
               | <Around>
...
The Joinpoint is a crucial entity of AspectJ and Aspect-Oriented Programming in the sense that it enables a sytematic categorization of some points of a program's execution. Formally, a Joinpoint is a "well-defined point in the execution of a program." \cite{LangDeats}. The keyword here is \textit{well-defined} - without formally defining which points are in a program are valid/invalid joinpoints, it would be extremely difficult for a developer to understand and reason about an Aspect-oriented system \cite{aop-ECCOP}. Figure 2 details the joinpoints which are enabled by AspectJ.

\begin{figure}
    \centering
    \includegraphics[width=8cm]{joinpoints.png}
    \caption{available AspectJ Joinpoints}
    \label{fig:2}
\end{figure}

Notice that we already saw an example of constructor execution in our \verb InstantiationAspect  with our pointcut \verb new_exec(). The wide variety of joinpoints enable a developer to target a variety of points in their programs where they want to execute some cross-cutting code. A classic example would be the update logic for the observer design pattern. If we had a GUI interface with some capability for drawing shapes, it is very well likely we could have some method such as 
\begin{verbatim}
...
\end{verbatim}
and the circumstances during which the match will occur, method \textit{execution} vs method \textit{call}.

\subsection{Exception Handling (within and handler)}
...
\end{verbatim}
With the \verb null_handler  pointcut, we effectively perform a match on the code in the class \verb Main  which handles exceptions of type \verb NullPointerException.
\subsection{Advice}
With some basic background in the Aspect, Joinpoint, and Pointcut, we now arrive at \textit{Advice}. Without Advice, there would be nothing determining how our code runs at every joinpoint selected by a pointcut. Therefore, we use Advice to communicate exactly how the code should execute and run to address the cross-cutting concern. We return to the informal EBNF describing the types of Advice enabled by AspectJ
\begin{verbatim}
    <Advice> ::= <Before>
               | <Around>
               | <After>
...
\end{verbatim}
 ...
 \end{verbatim}
 This is an example of a cross-cutting concern which is an ideal candidate for refactoring using \verb around , since they represent calls which will happen in pairs which close the \verb concurrent_access  joinpoint between themselves. We refactor \verb MutexAspect  to make use of \verb around  like so
 \begin{verbatim}
...
\end{figure}
 ...
 \begin{figure}
    ...
    \caption{A system with an encapsulated cross-cutting concern}
    \label{fig:1}
\end{figure}
